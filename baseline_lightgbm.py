...
=====================
Baseline LightGBM classifier for household COVID-19
...
LightGBM (n_estimators=200, max_depth=8) classifier on the same stratified
...
Outputs written to ``LightGBM_results_Full/``
-----------------------------------------
- ``LightGBM_results_Full/household_lightgbm_summary.csv``       per-fold metric summary
- ``LightGBM_results_Full/household_lightgbm_top50_features.csv`` mean feature importances
- ``LightGBM_results_Full/household_lightgbm_full_results.json``  complete result dump
- ``LightGBM_results_Full/lgbm_model_fold_{k}.joblib``       serialised model (k=1..5)
...
# ==================== CONFIGURATION ====================
output_dir = 'LightGBM_results_Full/'
...
# ==================== MAIN TRAINING LOOP ====================
print("\n" + "="*80)
print(f"LightGBM Training with Imbalance Handling: {IMBALANCE_STRATEGY.upper()}")
...
for fold in range(1, 6):
    ...
    model_path    = os.path.join(output_dir, f'lgbm_model_fold_{fold}.joblib')
    joblib.dump(model, model_path)
    features_path = os.path.join(output_dir, f'lgbm_features_fold_{fold}.json')
...
summary_path = os.path.join(output_dir, 'household_lightgbm_summary.csv')
summary_df.to_csv(summary_path, index=False)

importance_path = os.path.join(output_dir, f'household_lightgbm_top{top_n}_features.csv')
top_importance.to_csv(importance_path, index=False)

full_json_path = os.path.join(output_dir, 'household_lightgbm_full_results.json')
...
# ==================== PRINT SUMMARY ====================
print("\n" + "="*80)
print(f"LightGBM (IMBALANCE: {IMBALANCE_STRATEGY.upper()}) — 5-FOLD CV RESULTS")
